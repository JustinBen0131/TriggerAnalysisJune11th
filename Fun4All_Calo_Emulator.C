...
void Fun4All_Calo_Emulator(const string &fname1, int nEvents = 0, const string &outputFilename = "test.root",  const string &dstOutputFilename = "output/DST_EMULATOR.root")
{ 
...
//  sprintf(outfile_hist, "HIST_TRIGGER_QA-%08d-%04d.root", runnumber, 0);
 // string fulloutfile_hist = outfile_hist;
    ...
    Fun4AllOutputManager *out = new Fun4AllDstOutputManager("TriggerOut", dstOutputFilename.c_str());
    out->UseFileRule();
